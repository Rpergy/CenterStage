...
//        commented because testing vision code
        waitForStart();
        Trajectory start_centerSpike = new Trajectory(new Pose(11.5, 63, Math.toRadians(-90)))
                ...
                .lineTo(new Pose(20, 36, Math.toRadians(-135)))
        ...
        Trajectory centerSpike_canvas = new Trajectory()
        ...
        Trajectory start_leftSpike = new Trajectory(new Pose(11.5, 63, Math.toRadians(-90)))
                .action(() -> Actuation.setTilt(ActuationConstants.Extension.tiltPresets[3]))
                .action(() -> Actuation.setWrist(ActuationConstants.Claw.wristIntake))
                .lineTo(new Pose(19.5, 45, Math.toRadians(-90)))
                .action(() -> Actuation.setRClaw(ActuationConstants.Claw.open))
                .action(() -> sleep(500))
                .action(() -> Actuation.setTilt(ActuationConstants.Extension.tiltStacks))
                .action(() -> Actuation.setWrist(ActuationConstants.Claw.wristIntake - 0.03));

        Trajectory leftSpike_canvas = new Trajectory()
                .lineTo(new Pose(18.5, 52, Math.toRadians(-90)))
                .action(() -> Actuation.setTilt(ActuationConstants.Extension.tiltPresets[0]))
                .action(() -> Actuation.setWrist(ActuationConstants.Claw.wristDeposit))
                .lineTo(FieldConstants.Blue.Canvas.left, 0.3, ActuationConstants.Autonomous.turnSpeed)
                .action(() -> sleep(200))
                .action(() -> Actuation.setLClaw(ActuationConstants.Claw.open))
                .action(() -> sleep(500));

        Trajectory start_rightSpike = new Trajectory(new Pose(11.5, 63, Math.toRadians(-90)))
                .lineTo(new Pose(17, 60, Math.toRadians(-90)))
                .action(() -> Actuation.setTilt(ActuationConstants.Extension.tiltPresets[3]))
                .action(() -> Actuation.setWrist(ActuationConstants.Claw.wristIntake))
                .lineTo(new Pose(20, 36, Math.toRadians(-180)))
                .lineTo(new Pose(13.5, 36, Math.toRadians(-180)))
                .action(() -> Actuation.setRClaw(ActuationConstants.Claw.open))
                .action(() -> sleep(500))
                .action(() -> Actuation.setTilt(ActuationConstants.Extension.tiltStacks))
                .action(() -> Actuation.setWrist(ActuationConstants.Claw.wristIntake - 0.03));

        Trajectory rightSpike_canvas = new Trajectory()
                .lineTo(new Pose(15, 36, Math.toRadians(-180)))
                .action(() -> Actuation.setTilt(ActuationConstants.Extension.tiltPresets[0]))
                .action(() -> Actuation.setWrist(ActuationConstants.Claw.wristDeposit))
                .lineTo(FieldConstants.Blue.Canvas.right, 0.3, ActuationConstants.Autonomous.turnSpeed)
                .action(() -> sleep(500))
                .action(() -> Actuation.setLClaw(ActuationConstants.Claw.open))
                .action(() -> sleep(500));
        ...
        if (Math.max(Math.max(left-0.3, right-0.3), middle-0.2) == middle-0.2) {
            start_centerSpike.run();
            centerSpike_canvas.run();
        }
        else if (Math.max(Math.max(left-0.3, right-0.3), middle-0.2) == left-0.3) {
            start_leftSpike.run();
            leftSpike_canvas.run();
        }
        else if (Math.max(Math.max(left, right), middle) == right) {
            start_rightSpike.run();
            rightSpike_canvas.run();
        }
        canvas_farStack.run();
        farStack_Canvas.run();
        canvas_park.run();
